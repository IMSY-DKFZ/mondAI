...
At the moment the user is responsible for **normalizing** the images to the [0, 1] value range. Internally the image values are scaled to the expected value range of each metric. Metrics inform the user if values are outside of valid ranges by throwing errors, and sometimes even give warnings if they detect that for example all values are between [0, 1] but should be between [0, 255].

...
Converted by Sören Dittmer, Clemens Karner and Anna Breger (through project FWF T1307-N/Anna Breger) from the original
Python tensorflow implementation written by David Neumann and from the original MATLAB implementation written by Rafael
Reisenhofer.

Please note that this implementation requires images scaled to [0,1], while the original implementation expects images
with image values in [0,255].
...
def haarpsi(ref, deg, C, α, preprocess_with_subsampling=True):
    ...
    Parameters:
        Mandatory:
            ...
            C: float
                Natural images: 30
                Medical images: 5
                A paramter of the algorithm, which has been experimentally determined for natural and medical
                  image data sets.
                Range: [5, 100]
            α: float
                Natural images: 4.2
    ...
    # print("torch", torch.sum(ref), torch.sum(deg), torch.min(ref), torch.max(ref), torch.min(deg), torch.max(deg))
    ...
    if is_color_image:
        coefficients_ref_i = torch.abs(
            _convolve2d(ref_i, torch.ones((2, 2), device=ref_i.device, dtype=torch.float64) / 4.0)
        )
        coefficients_deg_i = torch.abs(
            _convolve2d(deg_i, torch.ones((2, 2), device=deg_i.device, dtype=torch.float64) / 4.0)
        )
        coefficients_ref_q = torch.abs(
            _convolve2d(ref_q, torch.ones((2, 2), device=ref_q.device, dtype=torch.float64) / 4.0)
        )
        coefficients_deg_q = torch.abs(
            _convolve2d(deg_q, torch.ones((2, 2), device=deg_q.device, dtype=torch.float64) / 4.0)
        )
    ...
    local_similarities = torch.zeros(n_channels, n_samples, 1, height, width, device=ref_y.device, dtype=torch.float64)
    weights = torch.zeros(n_channels, n_samples, 1, height, width, device=ref_y.device, dtype=torch.float64)

    for orientation in [0, 1]:
        weights[orientation] = _get_weights_for_orientation(
            coeffs_deg_y, coeffs_ref_y, n_scales, orientation, n_samples, height, width
        )
    ...
    def logit(value, α):
        return torch.log(value / (1 - value)) / α
...
def _subsample(image):
    kernel = torch.ones(2, 2, device=image.device, dtype=torch.float64) / 4.0
...
def _get_haar_filter(scale, device):
    haar_filter = 2**-scale * torch.ones(2**scale, 2**scale, device=device, dtype=torch.float64)
    haar_filter[: haar_filter.shape[0] // 2, :] = -haar_filter[: haar_filter.shape[0] // 2, :]
    return haar_filter


def _haar_wavelet_decompose(image, number_of_scales):
    coefficients = torch.zeros(2 * number_of_scales, *image.shape, device=image.device, dtype=torch.float64)

...
class NoReferenceMetric(Metric, ABC):
    ...
    def __call__(
        ...
        if compare_implementations:
            scores = {
                library_name: self._call_with_vectorization(image, dims=dims, compute_function=implementation)
                for library_name, implementation in self._other_implementations().items()
            }
            scores["mondAI"] = self._call_with_vectorization(image, dims=dims, compute_function=self._compute)
            return scores

        else:
            return self._call_with_vectorization(image, dims=dims, compute_function=self._compute)

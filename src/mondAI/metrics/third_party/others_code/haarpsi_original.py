...
Converted by David Neumann from the original MATLAB implementation written by Rafael
Reisenhofer.
...
def haar_psi(reference_image, distorted_image, preprocess_with_subsampling=True):
    """Calculates the HaarPSI perceptual similarity index between the two specified
    images.
    ...
    if is_numpy(reference_image) and is_numpy(distorted_image):
        ...
        raise ValueError(
            "The reference or the distorted image is neither a NumPy array, nor a TensorFlow tensor or variable. "
            "There are only NumPy and TensorFlow implementations available."
        )


def haar_psi_numpy(reference_image, distorted_image, preprocess_with_subsampling=True):
    """Calculates the HaarPSI perceptual similarity index between the two specified
    images. This implementation uses NumPy.
    ...
    # print(
    #     "numpy",
    #     numpy.sum(reference_image),
    #     numpy.sum(distorted_image),
    #     numpy.min(reference_image),
    #     numpy.max(reference_image),
    #     numpy.min(distorted_image),
    #     numpy.max(distorted_image),
    # )
    ...
    if is_color_image:
        reference_image_y = (
            0.299 * reference_image[:, :, 0] + 0.587 * reference_image[:, :, 1] + 0.114 * reference_image[:, :, 2]
        )
        distorted_image_y = (
            0.299 * distorted_image[:, :, 0] + 0.587 * distorted_image[:, :, 1] + 0.114 * distorted_image[:, :, 2]
        )
        reference_image_i = (
            0.596 * reference_image[:, :, 0] - 0.274 * reference_image[:, :, 1] - 0.322 * reference_image[:, :, 2]
        )
        distorted_image_i = (
            0.596 * distorted_image[:, :, 0] - 0.274 * distorted_image[:, :, 1] - 0.322 * distorted_image[:, :, 2]
        )
        reference_image_q = (
            0.211 * reference_image[:, :, 0] - 0.523 * reference_image[:, :, 1] + 0.312 * reference_image[:, :, 2]
        )
        distorted_image_q = (
            0.211 * distorted_image[:, :, 0] - 0.523 * distorted_image[:, :, 1] + 0.312 * distorted_image[:, :, 2]
        )
    else:
    ...
    similarity = (
        logit(numpy.sum(sigmoid(local_similarities[:], alpha) * weights[:]) / numpy.sum(weights[:]), alpha) ** 2
    )
...
def haar_psi_tensorflow(reference_image, distorted_image, preprocess_with_subsampling=True):
    """Calculates the HaarPSI perceptual similarity index between the two specified
    images. This implementation uses TensorFlow.
    ...
    # Calculates the final score
    similarity = (
        logit(tf.reduce_sum(sigmoid(local_similarities[:], alpha) * weights[:]) / tf.reduce_sum(weights[:]), alpha) ** 2
    )
...
def subsample(image):
    """Convolves the specified image with a 2x2 mean filter and performs a dyadic
    subsampling step. This simulates the typical distance between an image and its
    viewer.
    ...
    if is_numpy(image):
        ...
        raise ValueError(
            "The image is neither a NumPy array, nor a TensorFlow tensor or variable. There are only NumPy and "
            "TensorFlow implementations available."
        )
...
def convolve2d(data, kernel, mode="same"):
    """Convolves the first input array with the second one in the same way MATLAB does.
    Due to an implementation detail, the SciPy and MATLAB implementations yield
    different results. This method rectifies this shortcoming of the SciPy
    implementation.
    ...
    if is_numpy(data) and is_numpy(kernel):
        ...
        # TensorFlow requires a 4D Tensor for convolution, the data has to be shaped
        # [batch_size, width, height, number_of_channels]
        ...
        raise ValueError(
            "Either the data or the kernel is neither a NumPy array, nor a TensorFlow tensor or variable."
            " There are only NumPy and TensorFlow implementations available."
        )
...
def haar_wavelet_decompose(image, number_of_scales):
    ...
    if is_numpy(image):
        ...
        raise ValueError(
            "The image is neither a NumPy array, nor a TensorFlow tensor or variable."
            " There are only NumPy and TensorFlow implementations available."
        )
...
def sigmoid(value, alpha):
    ...
    if is_numpy(value):
        ...
        raise ValueError(
            "The value is neither a NumPy array, nor a TensorFlow tensor or variable."
            " There are only NumPy and TensorFlow implementations available."
        )


def logit(value, alpha):
    """Applies the logit function to the specified value, which is the reverse of the
...
def is_numpy(value):
    """Determines whether the specified value is a NumPy value, i.e. an numpy.ndarray
    or a NumPy scalar, etc.
...
def is_tensorflow(value):
    """Determines whether the specified value is a TensorFlow value, i.e. an
    tensorflow.Variable or a tensorflow.Tensor, etc.
